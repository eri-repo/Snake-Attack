function CPreloader() {
    ...
    this.unload = function () {
        ...
        if (_oButStart && typeof _oButStart.unload === 'function') {
            try{
                _oButStart.unload();
            }catch(e){
                // ignore
    ...
    this.attachSprites = function () {
        ...
        if (!_oLoadingText) {
            ...
            // ensure text has expected styling and position
            _oLoadingText.font = "30px " + FONT_GAME;
            _oLoadingText.color = "#fff";
            _oLoadingText.x = CANVAS_WIDTH/2;
            _oLoadingText.y = CANVAS_HEIGHT/2 + 100;
            _oLoadingText.textBaseline = "alphabetic";
            _oLoadingText.textAlign = "center";
    ...
    this.refreshLoader = function (iPerc) {
        // If UI hasn't been attached yet, create a minimal loading text so we can show progress
        if (!_oLoadingText) {
            if (_oContainer) {
                _oLoadingText = new createjs.Text(iPerc + "%", "30px " + FONT_GAME, "#fff");
                _oLoadingText.x = CANVAS_WIDTH/2;
                _oLoadingText.y = CANVAS_HEIGHT/2 + 100;
                _oLoadingText.textBaseline = "alphabetic";
                _oLoadingText.textAlign = "center";
        ...
        if (iPerc === 100) {
            s_oMain._onRemovePreloader();
            if (_oButStart) _oButStart.setVisible(false);
            if (_oLoadingText) _oLoadingText.visible = false;
            if (_oProgressBar) _oProgressBar.visible = false;
        }

        if (_oMaskPreloader && typeof _iMaskWidth !== 'undefined'){
