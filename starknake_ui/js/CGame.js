function CGame(oData) {
    ...
    this._init = function () {
        ...
        _iScore = START_QUEUE_SNAKES[PLAYER];
        _iBestScore = 0;
        ...
        _oInterface.refreshScore(_iPointsEarned);
    ...
    this.snakesHeadQueueCollision = function (oSnake1, oSnake2) {
        ...
        for (var j = aQueue1.length - 2; j > 0; j--) {
            ...
            if (this.circleToCircleCollision(aQueue1[j].getPos(), oSnake2.getPos(), aQueue1[j].getDim().h, oSnake2.getDim().w)) {
                ...
                var iNewLength = oSnake1.getLengthQueue();
                _iScore = iNewLength;
                _oInterface.refreshScore(_iPointsEarned);
    ...
    this.onDiePlayerSnake = function () {
        stopSound("soundtrack");
        _oInterface.createEndPanel(_iScore, _iPointsEarned);
    };
    ...
    this.updateScoreFood = function () {
        ...
        _iPointsEarned++;
        _oInterface.refreshScore(_iPointsEarned);
        if (_iPointsEarned > _iBestScore) {
            _iBestScore = _iPointsEarned;
