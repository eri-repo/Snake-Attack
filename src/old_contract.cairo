...
pub mod Starknake {
    ...
    const STRK_TOKEN_ADDRESS: ContractAddress = 0x04718f5a0fc34cc1af16a1cdee98ffb20c31f5cd61d6ab07201858f4287c938d.try_into().unwrap();
    ...
    #[abi(embed_v0)]
    ///on a view function, the get_caller_address() is address zero because the the transaction is
    ///free so no need you being the caller
    impl Starknake of IStarknake<ContractState> {

        fn player_registers(ref self: ContractState, player_address: ContractAddress, username: felt252) {

            assert(get_caller_address() == self.owner.read(), ONLY_OWNER); //only the owner can register players
            ...
            let player = UserInfo {
                ...
                username,
               // is_registered: true,
                //position: self.global_position.read() + 1, //this becomes the player's default rank
                highest_score: 0_u256, //default score is zero
                // registered_at: get_block_timestamp(),
                // updated: false,
            };

            //LEADERBOARD WILL BE ON THE BACKEND
            // self
            //     .leaderboard //add the player to the leaderboard
            //     .at((self.global_position.read()).try_into().unwrap_or(0))
            //     .write(
            //         BoardInfo {
            //             player: player_address, username, score: 0_u256 //default score is zero
            //         },
            //     );

            //increase the global ranking by 1
            // self.global_position.write(self.global_position.read() + 1);
        ...
        fn update_player_score(
            ...
            // let index = (player.position - 1).try_into().unwrap_or(0);
            // let mut board = self.leaderboard.get(index).unwrap().read();
            // board.score = board.score + current_score;

            player.highest_score = player.highest_score + current_score;

            //save board and player info
            // self.leaderboard.get(index).unwrap().write(board);
        ...
        //on the frontend, the sorting will be done and the first 20 on the leaderboard will be shown
        //this is to avoid expensive transaction of sorting onchain
        ...
        //TO THE FRONTEND AND SAVED IN DATABASE => ONLY CREATE CHALLENGE FUNCTION WILL BE ON-CHAIN FOR CONCLUDED CHALLENGES
        //Player challenges another player
        //the challenged can either accept the challenge or counter challenge the challenger with his own challenge condition
        //if the challenged accepts the challenger's condition, the challenge goes on but if the challenged counter
        //challenge the challenger, the challenger can accept the counter challenge 
        //(the counter challenge info will be used to update the original challenge) or updates the original challenge
        //and send it back to the challenged.. this back and forth could continue until there's an agreement.
        ...
        //this function is called when the challenged accepts the challenger's challenge without counter challenging
        // the function is called to update the challenge status to ACCEPTED
        // and the deducted points are locked
        //on the frontend, the players will stake STRK tokens during this period so they can play the game
        ...
        //THIS WILL BE DONE ON THE FRONTEND AND WHEN THE CHALLENGE IS COMPLETED/SETTLED, WE WILL SEND THE COMPLETED CHALLENGE ON-CHAIN
        //i will still need to work on this function as the caller will have to be us and not the players
        // the owner will be the caller of this function to settle the challenge and this will be set up automatically
        //at the end of the game, there will be a check if the game is completed and on the frontend, the winner will
        //be determined and once a winner is determined, this function will be called and the winner will be determined on-cahin also
